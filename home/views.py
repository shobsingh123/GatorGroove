...
import requests
from GatorGroove import settings
from django.core.mail import send_mail
from django.template.loader import render_to_string
from django.utils.html import strip_tags
from django.template import RequestContext
import openai
API_KEY = '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'
...
def generate_text(request):
    ...
    if request.method == 'POST':
        ...
        prompt=text 
        '''
    prompt = "Tell me about the latest events at University of Florida campus"
    openai.api_key = API_KEY
    response = openai.Completion.create(
        engine="davinci",
        prompt=prompt,
    ...
    text = response.choices[0].text
    return render(request, 'generate_text.html', {'text': text})
...
def contact(request):
    if request.method == "POST":
        ...
        if result['success']:
                ...
                send_mail('Registration Completed | GatorGroove',  strip_tags(message) , 'arnav.gaur@ufl.edu', [email], fail_silently=False, html_message=message)
                return redirect('home')
        else:
